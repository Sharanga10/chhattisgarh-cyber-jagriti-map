...
<body>
    ...
    <div class="app-workspace">
        ...
        <div class="toast-notification" id="toastSync" role="status" aria-live="polite" title="क्लिक करके बंद करें (Click to dismiss)">
            <span style="display:inline-block; width:8px; height:8px; border-radius:50%; background:#22c55e; flex-shrink:0;"></span>
            <span style="flex:1;">लाइव सिंक सक्रिय: {format_indian(total_events)} कार्यक्रम पूर्णतः सत्यापित एवं अपडेटेड हैं।</span>
            <span style="opacity:0.6; font-size:11px; margin-left:6px; cursor:pointer;" title="बंद करें">✕</span>
        </div>
    ...
    <script>
        ...
        if (toastSync) {{
            toastSync.addEventListener('click', () => {{
                toastSync.classList.remove('show');
                if (toastTimeout) clearTimeout(toastTimeout);
            }});
        }}

        function showSyncToast(msg, duration = 3000) {{
            if (!toastSync) return;
            if (msg) {{
                const msgSpan = toastSync.querySelector('span:nth-child(2)');
            ...
            try {{
                ...
                if (!feed) {{
                    if (isManual) {{
                        ...
                        showSyncToast(`लाइव पोर्टल सिंक सक्रिय: ${{formatIN(curTotal)}} कार्यक्रम पूर्णतः सत्यापित हैं।`, 3000);
                    }}
                    return;
                }}
                ...
                if (anyChanged || (feed.total_events && feed.total_events !== stateMasterEvents)) {{
                    ...
                    // Subtle indicator pulse on live map badge (silent background sync)
                    const liveDot = document.querySelector('.live-badge');
                    if (liveDot) {{
                        liveDot.style.boxShadow = '0 0 14px #22c55e';
                        setTimeout(() => {{ liveDot.style.boxShadow = ''; }}, 1200);
                    }}
                }}

                // Toast notifications ONLY appear when user manually clicks "लाइव सिंक" button
                if (isManual) {{
                    showSyncToast(`लाइव सिंक 100% सत्यापित: ${{formatIN(stateMasterEvents)}} कार्यक्रम एवं ${{(stateMasterReach / 100000).toFixed(2)}} लाख नागरिक पूर्णतः अपडेटेड हैं।`, 3000);
                }}
            }} catch (err) {{
